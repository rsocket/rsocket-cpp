...
{
    EXPECT_EQ(rb_.write(MSG_TYPE_ID, srcBuffer_.data(), rb_.maxMsgLength() + 1), INVALID_ARGUMENT);
}
...
TEST_F(OneToOneRingBufferTest, shouldWriteToEmptyBuffer)
{
    ...
    ASSERT_EQ(rb_.write(MSG_TYPE_ID, srcBuffer_.data(), length), alignedRecordLength);
...
TEST_F(OneToOneRingBufferTest, shouldRejectWriteWhenInsufficientSpace)
{
    ...
    ASSERT_EQ(rb_.write(MSG_TYPE_ID, srcBuffer_.data(), length), INSUFFICIENT_SPACE);
...
TEST_F(OneToOneRingBufferTest, shouldInsertPaddingRecordPlusMessageOnBufferWrap)
{
    ...
    ASSERT_EQ(rb_.write(MSG_TYPE_ID, srcBuffer_.data(), length), tail + alignedRecordLength + ALIGNMENT);
...
{
    uint32_t count = rb_.read([&](std::int32_t id, const uint8_t *b, uint32_t length)
    {
...
TEST(OneToOneRingBufferConcurrentTest, shouldExchangeMessages)
{
    ...
    spscBuffer.fill(0);
    srcBuffer.fill(0);
    OneToOneRingBuffer rb(spscBuffer.data(), static_cast<uint32_t>(spscBuffer.size()));
    ...
    {
        const int readCount =
            rb.read([&counts](std::int32_t msgTypeId, const uint8_t *buffer, uint32_t length)
            {
