//
// Created by Yuri Schimke on 6/20/16.
//
...
#include <reactivesocket-cpp/test/simple/CancelSubscription.h>
...
namespace {
    ...
    public:
        ...
        Subscriber<Payload> &
          ...
          auto* subscription = new MemoryMixin<CancelSubscription>();
        ...
        void handleRequestSubscription(
          ...
          std::cout << "ServerRequestHandler.handleRequestSubscription " << request->moveToFbString() << "\n";
    ...
    public:
        ...
        virtual void connectionAccepted(int fd, const SocketAddress &clientAddr) noexcept override {
          std::cout << "connectionAccepted" << clientAddr.describe() << "\n";

          auto socket = folly::AsyncSocket::UniquePtr(new AsyncSocket(&eventBase_, fd));

          std::unique_ptr<DuplexConnection> connection = folly::make_unique<TcpDuplexConnection>(std::move(socket));
          std::unique_ptr<DuplexConnection> framedConnection = folly::make_unique<FramedDuplexConnection>(std::move(connection));
          std::unique_ptr<RequestHandler> requestHandler = folly::make_unique<ServerRequestHandler>();

          reactiveSocket_ = ReactiveSocket::fromClientConnection(std::move(framedConnection), std::move(requestHandler));
        }
...
int main(int argc, char *argv[]) {
  ...
  eventBase.runInEventBaseThreadAndWait([&callback, &eventBase, &serverSocket]() {
