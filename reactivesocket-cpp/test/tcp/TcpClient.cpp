...
DEFINE_string(address, "[::]:9898", "host:port to connect to");
...
int main(int argc, char *argv[]) {
  ...
  eventBase.runInEventBaseThreadAndWait([&callback, &reactiveSocket, &eventBase]() {
      ...
      std::unique_ptr<RequestHandler> requestHandler = folly::make_unique<ClientRequestHandler>();

      // TODO add framing

      reactiveSocket = ReactiveSocket::fromClientConnection(std::move(framedConnection), std::move(requestHandler));

      auto* subscriber = new MemoryMixin<PrintSubscriber>();

      reactiveSocket->requestSubscription(folly::IOBuf::copyBuffer("from client"), *subscriber);
  });
...
//  connection->setInput();
  ...
  eventBase.runInEventBaseThreadAndWait([&reactiveSocket]() {
