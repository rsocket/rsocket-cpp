...
// The processor does not coordinate backpressure for its subscribers and implements a weaker onSubscribe which calls requests kNoFlowControl from the incoming Subscriptions. This makes it possible to subscribe the PublishProcessor to multiple sources unlike the standard Subscriber contract. 
// If subscribers are not able to keep up with the flow control, they are terminated with MissingBackpressureException.
// The implementation of onXXX() and subscribe() methods are technically thread-safe but non-serialized calls to them may lead to undefined state in the currently subscribed Subscribers. 
template <typename T>
class PublishProcessor : public Flowable<T>, public Subscriber<T> {
  ...
  void subscribe(std::shared_ptr<Subscriber<T>> subscriber) override {
    ...
    void onSubscribe(std::shared_ptr<Subscription> subscription) override {
	  ...
	  if(publishers == kCompletedPublishers || publishers == kErroredPublishers) {
	  	subscription->cancel();
	  	return;
	  }
...
private:
	...
	std::shared_ptr<const PublishersVector> tryAddPublisher(std::shared_ptr<PublisherSubscription> subscriber) {
      while(true) {
	      auto oldPublishers = publishers_.copy();
	      if(oldPublishers == kCompletedPublishers || oldPublishers == kErroredPublishers) {
	      	return oldPublishers;
	      }

	      auto newPublishers = std::make_shared<PublishersVector>();
	      newPublishers->reserve(oldPublishers->size() + 1);
	      newPublishers->insert(newPublishers->begin(), oldPublishers->cbegin(), oldPublishers->cend());
	...
	void removePublisher(PublisherSubscription* subscriber) {
      while(true) {
	      ...
	      auto removingItem = std::find_if(oldPublishers->cbegin(), oldPublishers->cend(), [&](const auto& publisherPtr)
	      {
	      ...
	      auto newPublishers = std::make_shared<PublishersVector>();
	      newPublishers->reserve(oldPublishers->size() - 1);
	      newPublishers->insert(newPublishers->begin(), oldPublishers->cbegin(), removingItem);
	      newPublishers->insert(newPublishers->end(), std::next(removingItem), oldPublishers->cend());
   ...
   public:
   	PublisherSubscription(std::shared_ptr<Subscriber<T>> subscriber, PublishProcessor* processor)
   	: subscriber_(std::move(subscriber)), processor_(processor) {}

    // cancel may race with terminate(), but the PublishProcessor::removePublisher will
    // take care of that
    // the race with on{Next, Error, Complete} methods is allowed by the spec
    ...
    // terminate will never race with on{Next, Error, Complete} because they are all called
    // from PublishProcessor and terminate is called only from dtor
    void terminate() {
...
private:
   ...
   folly::Synchronized<std::shared_ptr<const PublishersVector>, std::mutex> publishers_;
...
template <typename T>
std::shared_ptr<const typename PublishProcessor<T>::PublishersVector> PublishProcessor<T>::kCompletedPublishers = std::make_shared<const PublishersVector>();

template <typename T>
std::shared_ptr<const typename PublishProcessor<T>::PublishersVector> PublishProcessor<T>::kErroredPublishers = std::make_shared<const PublishersVector>();
