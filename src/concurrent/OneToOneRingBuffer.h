...
struct RingBufferDescriptorDefn
{
    ...
    std::int8_t headCachePositionPad[(2 * CACHE_LINE_LENGTH) - sizeof(std::uint64_t)];
    ...
    std::int8_t correlationCounterPad[(2 * CACHE_LINE_LENGTH) - sizeof(std::uint64_t)];
};
#pragma pack(pop)

constexpr static const uint32_t TRAILER_LENGTH = sizeof(RingBufferDescriptorDefn);
...
inline static std::uint64_t makeHeader(std::uint32_t length, std::int32_t msgTypeId)
{
    return ((static_cast<std::uint64_t>(msgTypeId) & 0xFFFFFFFF) << 32) | (length & 0xFFFFFFFF);
}
...
public:
    ...
    OneToOneRingBuffer(uint8_t *buffer, uint32_t length) :
        ...
        descriptor_(reinterpret_cast<RingBufferDescriptor::RingBufferDescriptorDefn *>(buffer_ + capacity_)),
        tailPosition_(buffer_ + capacity_ + offsetof(RingBufferDescriptor::RingBufferDescriptorDefn, tailPosition)),
        headCachePosition_(buffer_ + capacity_ + offsetof(RingBufferDescriptor::RingBufferDescriptorDefn, headCachePosition)),
        headPosition_(buffer_ + capacity_ + offsetof(RingBufferDescriptor::RingBufferDescriptorDefn, headPosition)),
        correlationCounter_(buffer_ + capacity_ + offsetof(RingBufferDescriptor::RingBufferDescriptorDefn, correlationCounter)),
    ...
    position_t write(std::int32_t msgTypeId, const uint8_t *srcBuffer, uint32_t length) noexcept
    {
        if (msgTypeId < 1)
        {
        ...
        const uint32_t requiredCapacity = RingBufferDescriptor::align(recordLength, RingBufferDescriptor::ALIGNMENT);
        ...
        if (0 != padding)
        {
            putUInt64Ordered(
                buffer_ + recordIndex,
                RingBufferDescriptor::makeHeader(padding, RingBufferDescriptor::PADDING_MSG_TYPE_ID));
            recordIndex = 0;
        }

        ::memcpy(buffer_ + recordIndex + RingBufferDescriptor::HEADER_LENGTH, srcBuffer, length);
        putUInt64Ordered(buffer_ + recordIndex, RingBufferDescriptor::makeHeader(recordLength, msgTypeId));
    ...
    uint32_t read(F&& handler, uint32_t messageCountLimit) noexcept
    {
        ...
        while ((bytesRead < contiguousBlockLength) && (messagesRead < messageCountLimit))
        {
            ...
            const std::uint32_t recordLength = RingBufferDescriptor::headerRecordLength(header);
            ...
            bytesRead += RingBufferDescriptor::align(recordLength, RingBufferDescriptor::ALIGNMENT);

            const std::int32_t msgTypeId = RingBufferDescriptor::headerMessageTypeId(header);
            ...
            ++messagesRead;
            handler(msgTypeId, buffer_ + recordIndex + RingBufferDescriptor::HEADER_LENGTH, recordLength - RingBufferDescriptor::HEADER_LENGTH);
        }
...
private:
    ...
    inline static void checkCapacity(uint32_t capacity)
    {
        ...
        {
            throw std::invalid_argument("Capacity must be a positive power of 2 + TRAILER_LENGTH");
        }
    }
