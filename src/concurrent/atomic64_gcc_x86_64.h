...
inline void putUInt64Ordered(volatile std::uint8_t*  address, std::uint64_t value)
{
    thread_fence();
    *reinterpret_cast<volatile std::uint64_t *>(address) = value;
}

inline std::uint64_t getAndAddUInt64(volatile std::uint8_t* address, std::uint64_t value)
{
...
#define REACTIVESOCKET_DECL_ALIGNED(declaration, amt) declaration __attribute__((aligned(amt)))
