...
std::vector<T> run(Reference<Flowable<T>> flowable) {
  auto collector = Reference<CollectingSubscriber<T>>(
      new CollectingSubscriber<T>);
...
TEST(FlowableTest, JustFlowable) {
  ...
  EXPECT_EQ(run(Flowables::just({12, 34, 56, 98})),
            std::vector<int>({12, 34, 56, 98}));
  EXPECT_EQ(run(Flowables::just({"ab", "pq", "yz"})),
